...
- 已做 **12 個**：menu/title_pt/r_menu/cg_pt/sl_save/sl_load/bktitlen/edk_pt/sl_parts/sl_pt/soundk_pt(返回標題 nav)、
  **setting(設定畫面：全部標籤 音效設定/音樂/效果音/語音/男聲/女聲/文字顯示設定/跳過已讀文章/自動播放/畫面效果設定/視窗顏色/紅綠藍透明/範例/畫面效果，
  + 底部按鈕列 上頁/下頁/恢復預設/確定/取消/套用 主 2 態)**。backlog/baklog 純箭頭**無文字**。
- setting 心得：label 又暗又密、背景花 → 用**自適應偵測**(text = 比 local median bg 亮)抓 label bbox 最準;**flat-erase**(取 box 中位數填回,
  文字是少數)雖會在花紋 bg 留一塊平色但可讀。按鈕擦字要**蓋滿原文+略寬**(邊界會留殘字 頁/ﾄ)。多狀態/子頁的按鈕列(hover/pressed 額外態)未全做。
- **剩 soundk_pt 音樂室 ~14-16 首曲名**：sprite 是 2 欄×多軌×多態的密集 grid,40px 列距,但**每軌幾態(3 或 4)我反覆判讀不一致、grid 對映不穩**,
  且曲名**多為可讀漢字**(運命の一族/侯爵令嬢/破滅への階段/午後のテラス…)、音樂 gallery 低頻。硬做易留破圖,**暫不做以免 ship 壞圖**。
  → 結論:遊戲**功能完整可玩**,僅剩此一音樂室曲名(可讀漢字)為選配。
