...
- 已做 **8 個**：menu(指令選單)、title_pt(標題)、r_menu(右鍵金色選單)、cg_pt(CG)、sl_save/sl_load、
  bktitlen(返回標題確認)、edk_pt(結局選單「タイトルに戻る」×3)。backlog/baklog 是純箭頭圖示**無文字**。
  未做（密集多狀態，宜先確認譯名）：setting(設定滑桿)、soundk_pt(~20 BGM 曲名)、sl_pt+sl_parts(存讀檔按鈕：
  決定/キャンセル/メモ編集/文字全消去/閉じる/頁/未使用 + 確認對話框，有白/紅/金漸層多狀態)。
- 工具：`/tmp/uizoom.py <name> <zoom> [y0 y1]` 產生**座標格**(黃=x 青=y)讀像素座標；`/tmp/uiloc.py` 有
  decode_color/encode_blob 等 helper（⚠️ import 它會副作用重跑 title_pt，無害）。重打：`gcc_codec.repack_arc(orig,{name:bytes})`。
