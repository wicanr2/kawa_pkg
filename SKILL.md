...
引擎固定 640×480，字小是先天，**跟字型無關**：量過 OS 裡所有繁體字型
（Noto/WenQuanYi/Droid/UMing…）同一字級下字框大小幾乎一樣（差 <2%），換字型不會變大。
`gamescope`（能做視窗 nearest 縮放）在 Ubuntu 22.04 裝不起來（apt/snap 都沒有、要自己編）。

**最終方案（AppRun 預設）＝ xrandr nearest 整桌面縮放**：遊戲維持 640×480 視窗，
`xrandr --output <OUT> --scale SxS --filter nearest`（nearest = 銳利不糊）把整個桌面放大，
結束/中斷時 `--scale 1x1` 還原（trap EXIT/INT/TERM）。
- `SCALE` 預設 **0.42（≈2.4x，幾乎填滿）**；可用 `KAWA_SCALE` 覆寫。
- **下限約 0.41**：framebuffer 高 = 1200×SCALE 必須 ≥ 視窗 500px，否則字幕框底部被切。
  要更大只能引擎全螢幕（下方），但會糊。
- `KAWA_NOSCALE=1` = 原生 640×480 不縮放。
